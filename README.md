...
별도 로그인 페이지가 없습니다. 주소를 열면 **바로 실적이 보이고**, 로그인은 **화면 우측 상단**의 이메일·비밀번호 입력란에서 합니다. 로그인하면 그 자리가 계정명 + 역할 배지 + 로그아웃 버튼으로 바뀝니다.

로그인에 실패하거나 로그아웃해도 화면이 가려지지 않고 열람은 계속 유지됩니다.

### 로그인 유지 / 아이디 저장

우측 상단 입력란 아래 체크박스 두 개가 있고, 둘 다 기본으로 켜져 있습니다. 설정은 브라우저별로 기억됩니다.
